...
export class ResearchAIController {
  ...
  update(vehicle, vehicles, track, race = null, dt = 1 / 120) {
    ...
    const refSpeed = refData?.targetSpeed ?? refData?.envelopeSpeed ?? refData?.speed;
    if (Number.isFinite(refSpeed) && refSpeed > 10.0 && tacticalMode === 'PACE' && (vehicle.classKey === 'prototype' || !vehicle.classKey)) {
      ...
      // A reference lap is feed-forward guidance, never permission to violate
      // the live braking/trajectory envelope.  The previous max() could add
      // five percent precisely at corner entry, suppressing threshold braking
      // and launching the Prototype beyond the outside kerb.
      desiredSpeed = Math.min(desiredSpeed, scaledRef);
    }
    ...
    // Persist the deployed decision even when the expensive visual debugger is
    // disabled. GPT Racing's lap telemetry and race HUD consume this compact
    // state on every fixed step.
    this.lastDecision = {
      tacticalMode,
      tacticalReason,
      desiredSpeed,
      ...
      targetOffset: tacticalMode === 'DEFEND'
        ? finite(defDecision.desiredOffset, targetOffset)
        : finite(plannedTargetOffset, targetOffset),
      targetPos,
      targetId,
      committed,
      defending,
