...
/** Sample-free race soundscape with bounded continuous and transient nodes. */
export class SynthAudio {
  ...
  _createNoise(context, seconds = 2) {
    ...
    for (let i = 0; i < data.length; i += 1) data[i] = Math.sin(i * 12.9898) * 0.72;
    return buffer;
  }
  ...
  _create() {
    ...
    this.master = this._gain(context, this.muted ? 0 : 0.48);
    this.master.connect(context.destination);

    const engineFilter = context.createBiquadFilter();
    engineFilter.type = 'lowpass'; engineFilter.frequency.value = 850; engineFilter.Q.value = 1.3;
    ...
    gearFilter.type = 'bandpass'; gearFilter.frequency.value = 1400; gearFilter.Q.value = 0.75;
    ...
    this.pitLimiterOscillator = this._continuousOscillator(context, 'square', 92, this.pitLimiterGain, this.master);

    const noise = this._createNoise(context);
    this.skidGain = this._gain(context); this._continuousNoise(context, noise, 'bandpass', 1650, this.skidGain);
    this.scrubGain = this._gain(context); this._continuousNoise(context, noise, 'highpass', 420, this.scrubGain);
    this.curbGain = this._gain(context); this._continuousNoise(context, noise, 'bandpass', 165, this.curbGain);
    this.windGain = this._gain(context); this._continuousNoise(context, noise, 'lowpass', 410, this.windGain);
    this.ready = true;
  }
  ...
  update(vehicle = {}, dt = 0) {
    ...
    const tyreUtilisation = wheels.length ? Math.max(0, ...wheels.map((wheel) => finite(wheel?.utilisation))) : 0;
    ...
    const curbContact = wheels.reduce((peak, wheel) => Math.max(peak,
      finite(wheel?.curbContact) ? 1 : 0,
      wheel?.surface === 'curb' || wheel?.contactZone === 'curb' || wheel?.onCurb ? 1 : 0,
      finite(wheel?.contactHeight) > 0.018 ? 0.55 : 0,
      Math.abs(finite(wheel?.suspensionVelocity)) * 0.28
    ), 0);
    ...
    const pitLimiter = Boolean(vehicle.pitLimiterActive || vehicle.pit?.limiterActive || vehicle.pit?.state === 'LIMITER');
    this.pitLimiterOscillator.frequency.setTargetAtTime(78 + finite(vehicle.rpm) / 95 + (pitLimiter ? 38 : 0), time, 0.025);
    this.pitLimiterGain.gain.setTargetAtTime(pitLimiter ? 0.004 + throttle * 0.012 : 0.0001, time, 0.025);

    this.impactCooldown = Math.max(0, this.impactCooldown - safeDt);
    this.thumpCooldown = Math.max(0, this.thumpCooldown - safeDt);
    ...
    if (finite(vehicle.impact) > 0.24 && this.impactCooldown <= 0) { this._impact(finite(vehicle.impact)); this.impactCooldown = 0.16; }
    if (curbRumble > 0.46 && this.curbCooldown <= 0) { this._curbRumble(curbRumble); this.curbCooldown = 0.10; }
    const gear = finite(vehicle.gear, this.lastGear);
    if (gear !== this.lastGear && this.shiftCooldown <= 0) { this._shift(gear > this.lastGear, clamp(rpmFactor, 0.2, 1)); this.shiftCooldown = 0.11; }
    if ((gear < this.lastGear || (this.lastThrottle > 0.45 && throttle < 0.08 && finite(vehicle.rpm) > 3900)) && this.popCooldown <= 0) { this._pop(clamp(rpmFactor, 0.25, 1)); this.popCooldown = 0.19; }
    this.lastGear = gear;
    this.lastThrottle = throttle;
    this._updateAmbient(safeDt);
  }

  _updateAmbient(dt) {
    this.ambientClock += dt;
    if (this.ambientClock < this.nextAmbientAt) return;
    const mode = this.ambientIndex % 3;
    if (mode === 0) this._ambientHorn();
    if (mode === 1) this._marshalWarning();
    if (mode === 2) this._tracksidePA();
    this.ambientIndex += 1;
    this.nextAmbientAt = this.ambientClock + 10 + ((this.ambientIndex * 7) % 9);
  }
  ...
  _oneShot(type, frequency, peak, duration, endRatio = 0.34) {
    ...
    oscillator.start(); oscillator.stop(context.currentTime + duration + 0.02);
  }
  ...
  toggleMute() {
    ...
    if (this.master && this.context) this.master.gain.setTargetAtTime(this.muted ? 0 : 0.48, this.context.currentTime, 0.03);
    return this.muted;
  }
  ...
  dispose() {
    for (const source of this.continuousSources) {
      ...
      try { source.disconnect?.(); } catch { /* harmless during teardown */ }
    }
    this.continuousSources.length = 0;
    try { this.master?.disconnect?.(); } catch { /* harmless during teardown */ }
    this.context = null;
    this.master = null;
    this.ready = false;
  }
