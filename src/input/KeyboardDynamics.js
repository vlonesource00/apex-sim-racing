...
// controls. It deliberately does not alter vehicle state or track position.
export class KeyboardDynamics {
  ...
  update(raw = {}, vehicle = null, dt = 1 / 120) {
    ...
    // A keyboard request is a curvature request, not a direct high-speed wheel
    // angle.  At speed, cap the road wheel from a lateral-acceleration target
    // using the bicycle relation δ = atan(L * a_lat / v²).  The low-speed blend
    // deliberately retains manoeuvring authority without a v² singularity.
    ...
    // The old cap treated every car as a ~1 g mechanical-grip car, so its
    // steering angle collapsed with v² even while a prototype was gaining
    // several tonnes of usable aerodynamic load. Convert the live downforce
    // into an additional lateral-acceleration budget. Because this reads the
    // already-solved aero state it also naturally respects floor stall and
    // dirty-air front/rear load loss instead of granting artificial rotation.
    ...
    // Preserve a reserve for the tyre force model when the held key asks for
    // more curvature than the current body state can support.  The reserve is
    // based on measured sideslip/yaw, never on a direct yaw or position edit.
    ...
    // Intervene only when the car is rotating materially faster than the
    // driver's requested curvature or has developed large body slip. The old
    // absolute-yaw envelope could remove 72% of held steering in a healthy
    // high-speed corner, creating the reported "brick" response.
