...
import { RubberSurfaceRenderer } from './RubberSurfaceRenderer.js';

function makeRoadTexture() {
  const canvas = document.createElement('canvas');
  canvas.width = canvas.height = 512;
  const context = canvas.getContext('2d');
  context.fillStyle = '#39403d';
  context.fillRect(0, 0, 512, 512);
  for (let i = 0; i < 4200; i += 1) {
    const shade = 42 + (i * 29) % 32;
    context.fillStyle = 'rgba(' + shade + ',' + (shade + 5) + ',' + shade + ',0.16)';
    const size = 1 + (i % 4);
...
function makeGrassTexture() {
  ...
  const context = canvas.getContext('2d');
  context.fillStyle = '#315535';
  context.fillRect(0, 0, 256, 256);
  for (let i = 0; i < 2400; i += 1) {
    const dark = i % 3 === 0;
    context.fillStyle = dark ? 'rgba(18,45,24,0.22)' : 'rgba(116,151,75,0.16)';
...
function makeCheckerTexture() {
  ...
  for (let y = 0; y < 8; y += 1) {
    for (let x = 0; x < 8; x += 1) {
      context.fillStyle = (x + y) % 2 ? '#151917' : '#edf0e7';
...
// One lateral-placement rule keeps every procedural layer on the same
// elevation/bank contract that physics exposes through Circuit samples.
...
// Pure render-side terrain query.  The track remains the physics authority;
// this only carves the decorative ground below the already-authored road,
// curb, runoff and barrier corridor.
export function terrainHeightAt(track, x, z) {
  ...
  // A small conservative margin keeps the green mesh below the visible
  // banked track even when neighbouring samples interpolate across a seam.
...
function pointYaw(point) {
  return Math.atan2(point.tangent.x, point.tangent.z);
}
...
function makeSign(text, width = 9) {
  ...
  context.fillStyle = '#e9ff45';
  context.fillRect(28, 28, canvas.width - 56, 12);
  context.fillStyle = '#edf7e9';
  context.font = '900 118px Arial, sans-serif';
  context.textAlign = 'center';
  context.textBaseline = 'middle';
  context.fillText(text, canvas.width / 2, 153);
...
export class CircuitEnvironment {
  constructor(scene, track) {
    ...
    this.rubberRenderer = new RubberSurfaceRenderer(this.root, this.track);
  }

  _createSkyAndTerrain() {
    const authored = this.track.scenario?.controlPoints ?? this.track.samples ?? [];
    const extent = Math.max(340, ...authored.map((point) => Math.max(Math.abs(point.x ?? 0), Math.abs(point.z ?? 0)))) + 170;
    this.extent = extent;
    this.scene.fog = new THREE.FogExp2('#aec0a8', 0.00205);
    const sky = new THREE.Mesh(new THREE.SphereGeometry(extent * 2.25, 32, 20), new THREE.ShaderMaterial({
      side: THREE.BackSide,
      uniforms: {
        topColor: { value: new THREE.Color('#4a789a') },
        horizonColor: { value: new THREE.Color('#c9d6b4') },
        bottomColor: { value: new THREE.Color('#405b42') }
      },
      ...
      fragmentShader: 'uniform vec3 topColor; uniform vec3 horizonColor; uniform vec3 bottomColor; varying vec3 vWorld; void main(){ float h=normalize(vWorld).y; vec3 c=mix(horizonColor,topColor,smoothstep(-0.04,0.72,h)); c=mix(bottomColor,c,smoothstep(-0.25,0.09,h)); gl_FragColor=vec4(c,1.0); }'
    }));
    this.root.add(sky);
  ...
  _createLighting() {
    const hemisphere = new THREE.HemisphereLight('#dff4ff', '#263c2a', 2.1);
    this.root.add(hemisphere);
    const sun = new THREE.DirectionalLight('#fff1d2', 3.2);
    sun.position.set(-90, 145, 45);
    sun.castShadow = true;
  ...
  _createCircuitBed() {
    const road = new THREE.Mesh(roadGeometry(this.track), new THREE.MeshStandardMaterial({
      map: makeRoadTexture(),
      color: '#4a504d',
      roughness: 0.91,
      metalness: 0.015,
    ...
    const shoulder = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: '#d8d5c7', roughness: 0.84, side: THREE.DoubleSide })
    );
    ...
    const runoff = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: '#657078', roughness: 0.95, side: THREE.DoubleSide })
    );
    ...
    const outerVerge = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: '#45683f', roughness: 1, side: THREE.DoubleSide })
    );
  ...
  _createCurbs() {
    const red = [];
    const white = [];
    const segmentLength = this.track.length / this.track.samples.length * 2.15;
    for (let i = 0; i < this.track.samples.length; i += 2) {
      const point = this.track.samples[i];
  ...
  _createPitAndStart() {
    ...
    const pit = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: '#3e4542', roughness: 0.93, side: THREE.DoubleSide })
    );
    ...
    const pitLine = new THREE.Mesh(
      segmentGeometry(this.track, pitStart + 4, pitEnd - 3, this.pitSide * (this.track.roadHalfWidth + this.track.curbWidth + 1.1), this.pitSide * (this.track.roadHalfWidth + this.track.curbWidth + 1.26), 0.068),
  ...
  _createBarriersAndFence() {
    ...
    const segmentLength = this.track.length / this.track.samples.length * 3.5;
    for (let i = 0; i < this.track.samples.length; i += 3) {
      const point = this.track.samples[i];
      for (const side of [-1, 1]) {
    ...
    addInstanced(this.root, new THREE.BoxGeometry(0.2, 0.76, 3.4), new THREE.MeshStandardMaterial({ color: '#d1d8d0', roughness: 0.4, metalness: 0.7 }), rails, { castShadow: true });
    addInstanced(this.root, new THREE.BoxGeometry(0.045, 1.55, 3.4), new THREE.MeshStandardMaterial({ color: '#2d4039', roughness: 0.72, metalness: 0.2, transparent: true, opacity: 0.78 }), fences, { receiveShadow: true });
    addInstanced(this.root, new THREE.BoxGeometry(0.13, 2.1, 0.13), new THREE.MeshStandardMaterial({ color: '#59645e', roughness: 0.5, metalness: 0.62 }), posts, { castShadow: true });
  }

  _createFallbackProps() {
    const steel = new THREE.MeshStandardMaterial({ color: '#303d3a', metalness: 0.7, roughness: 0.36 });
    const seat = new THREE.MeshStandardMaterial({ color: '#164d69', roughness: 0.75 });
    ...
    const sign = makeSign('APEX // 73', 8.2);
  ...
  installAssets(assets) {
    ...
    for (let i = 0; i < this.track.samples.length; i += 11) {
      const point = this.track.samples[i];
      const side = i % 3 ? -1 : 1;
      place('PROP_TREE', point.s, side, 29 + (i % 5) * 2.6, 0.8 + (i % 4) * 0.1, i * 0.13);
    }
    for (let i = 0; i < this.track.samples.length; i += 10) {
      const point = this.track.samples[i];
  ...
  update(dt = 0) {
    return this.rubberRenderer?.update(dt) ?? false;
  }

  dispose() {
    this.rubberRenderer?.dispose();
    this.rubberRenderer = null;
