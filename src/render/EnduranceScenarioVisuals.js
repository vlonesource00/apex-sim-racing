...
import { ENDURANCE_PARK, sampleEnduranceParkAtFraction } from '../scenarios/EndurancePark.js';
...
/**
 * Pure placement planning keeps the substantial scenery deterministic and is
 * usable by tests or a loading screen without constructing any Three objects.
 */
export function createEndurancePlacementPlan(scenario = ENDURANCE_PARK) {
...
function trackSample(track, fraction, scenario) {
  ...
  const fallback = sampleEnduranceParkAtFraction(fraction, scenario);
  return {
    position: new THREE.Vector3(fallback.x, fallback.y, fallback.z),
    tangent: new THREE.Vector3(fallback.tangent.x, 0, fallback.tangent.z).normalize()
  };
}
...
/**
 * Browser-efficient endurance scenery. Most components are InstancedMesh
 * batches; authored Blender landmarks are optional and retain a procedural
 * fallback when the AssetLibrary has not finished loading.
 */
export class EnduranceScenarioVisuals {
  constructor(scene, track, scenario = ENDURANCE_PARK, assets = null) {
    this.scene = scene ?? null;
    this.track = track ?? null;
    this.scenario = scenario ?? ENDURANCE_PARK;
    this.assets = assets;
    this.root = new THREE.Group();
    this.root.name = 'ENDURANCE_PARK_SCENERY';
    this.plan = createEndurancePlacementPlan(this.scenario);
    this.geometry = [];
    this.materials = [];
    this.instances = [];
    this.animated = [];
  ...
  _batch(name, geometry, material, placements, configure) {
    ...
    mesh.name = `ENDURANCE_${name.toUpperCase()}`;
  ...
  _addLandmarks(steel, concrete, yellow, glass, lamp) {
    for (const landmark of this.plan.landmarks) {
      ...
      group.name = `ENDURANCE_${String(landmark.type).toUpperCase()}`;
    ...
    // A start/finish gantry is always present, even if authored assets are
    // unavailable, and is retained as a single small landmark group.
    const start = placementTransform(this.track, { fraction: this.scenario.start?.finishFraction ?? 0, lateral: 0 }, this.scenario);
    const gantry = new THREE.Group();
    gantry.name = 'ENDURANCE_START_GANTRY';
  ...
  _authoredLandmark(landmark) {
    ...
    clone.name = `ENDURANCE_AUTHORED_${propName}`;
    this.root.add(clone);
    return true;
  }
  ...
  update(dt, vehicles = [], race = null) {
    ...
    // Keep the signature integration-friendly; the existing scenery does not
    // own vehicle or race state and intentionally performs no per-frame allocs.
    void vehicles;
    void race;
  }
